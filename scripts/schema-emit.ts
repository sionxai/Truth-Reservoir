...
const openapi = {
  ...
  paths: {
    ...
    "/api/v2/topics.json": {
      get: {
        summary: "Fetch the deterministic topic collection registry",
        description:
          "Derived from tags at build time. Each topic aggregates propositions sharing a tag, ordered by event date parsed from sixW.when (제14). Topics are a derived navigation layer and are not stored in Cert originals.",
        responses: {
          "200": {
            description: "Static derived topic registry",
            content: {
              "application/json": {
                schema: {
                  $ref: "#/components/schemas/TopicRegistry"
                }
              }
            }
          }
      ...
      TopicRegistry: {
        ...
        properties: {
          ...
          topics: {
            ...
            items: {
              type: "object",
              required: ["tag", "path", "count", "dateRange", "propositionIds"],
              properties: {
                tag: { type: "string" },
                path: { type: "string" },
